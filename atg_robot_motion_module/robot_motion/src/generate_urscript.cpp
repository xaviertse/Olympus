...
//      ss << "\t  end #end while\n";
      ss << "\t  while True:\n";
      ss << "\t    ft    = get_tcp_force()\n";
      ss << "\t    ft_B  = p[ft[0],ft[1],ft[2],0,0,0]\n";
      ss << "\t    tcp   = get_actual_tcp_pose()\n";
      ss << "\t    tcp_B = p[tcp[0],tcp[1],tcp[2],0,0,0]\n";
      ss << "\t    force_T = pose_trans(pose_inv(tcp),pose_trans(tcp_B,ft_B))\n";
      ss << "\t    g_Fx = force_T[0]\n";
      ss << "\t    g_Fy = force_T[1]\n";
      ss << "\t    g_Fz = force_T[2]\n";
      ss << "\t    g_Mx = force_T[3]\n";
...
//      ss << "\t  end\n";
      ss << "\t  zero_ftsensor()#doesnt seem to do anything\n";
      ss << "\t  g_Fz_zero = get_sensor_fz()\n";
      ss << "\tend\n";
...
//      ss << "\t      if not (get_sensor_fz() >= -f_value_):\n";
      ...
      ss << "\t    g_thread_flag_4 = 2\n";
      ss << "\t  end\n";
      ss << "\t  F_z = get_sensor_fz()-g_Fz_zero\n";
