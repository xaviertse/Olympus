...
UR10ERobotModel::UR10ERobotModel()
{
...
bool UR10ERobotModel::initialize(const std::string &robot_description, const std::string& group_name,
                          const std::string& world_frame,const std::string& tcp_frame)
{
  check_collisions_ = true;
  if(!MoveitStateAdapter::initialize(robot_description,group_name,world_frame,tcp_frame))
  {
    ROS_ERROR_STREAM("MoveitStateAdapter from within UR10E robot model failed to initialize");
    return false;
  }


  if(!ur10e_ur_kinematics::URKinematicsPlugin::initialize(robot_description, group_name, UR10E_BASE_LINK,UR10E_TIP_LINK, 0.001))
  {
    ROS_ERROR_STREAM("URKinematicsPlugin from within UR10E robot model failed to initialize");
    return false;
  }
  ...
  ROS_WARN_STREAM("UR10E Descartes Robot Model initialized");

  return true;
}


bool UR10ERobotModel::getAllIK(const Eigen::Affine3d &pose, std::vector<std::vector<double> > &joint_poses) const
{
  using namespace ur10e_ur_kinematics;
  ...
  ROS_DEBUG_STREAM("ur10e custom ikfast found "<<num_sols<<" solutions");
...
bool UR10ERobotModel::getIK(const Eigen::Affine3d &pose, const std::vector<double> &seed_state,
